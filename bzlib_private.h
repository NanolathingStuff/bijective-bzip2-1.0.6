...
extern UInt32 BZ2_crc32Table[256];	//crctable.c
...
typedef
   struct {
      ...
      UInt32*  arr1;		//used in: BZ_API(BZ2_bzCompressInit),BZ_API(BZ2_bzCompressEnd),
      UInt32*  arr2;		//		BZ2_blockSort,	BZ_API(BZ2_bzCompressEnd)
      UInt32*  ftab;		//fallbackSort, used for undoing burrows-wheeler transform
      Int32    origPtr;
...
typedef
   struct {
      ...
      /* map of bytes used in block */
      Int32    nInUse;			//bytes in use
      Bool     inUse[256];		//wich ones
      Bool     inUse16[16];		//wich ones
